...
def friends():
    ...
    query = "SELECT *, Temp2.user2 as followed FROM (SELECT * FROM Users WHERE username!=?) as Temp1 LEFT JOIN (SELECT * FROM Following WHERE user1=?) AS Temp2 ON Temp1.username=Temp2.user2"
    cur = connection.execute(query, (Project507.app.config['CURRENT_USER'], Project507.app.config['CURRENT_USER']))
